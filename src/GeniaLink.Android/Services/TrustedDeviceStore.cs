...
internal sealed class TrustedDeviceStore
{
    ...
    public TrustedDeviceStore(global::Android.Content.Context context, Action<string>? log)
    {
        ...
        _path = Path.Combine(filesDirectory, "trusted-devices.json");
        _log = log;
        _identityRegistry = new TrustedIdentityRegistry(Path.Combine(filesDirectory, "trusted-identities.json"), log);
    ...
    public bool Remove(Guid deviceId)
    {
        lock (_gate)
        {
            ...
            // Preserve revocation fail-closed across compatibility-store recovery, while
            // still allowing an explicit Forget action to clear a registry-only record.
    ...
    private static void TryDeleteTempFile(string path)
    {
        ...
        {
            // The final database remains authoritative. A stale temp file is ignored on load.
        }
